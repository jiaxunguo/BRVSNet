...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        # [20, 3, 224, 224]
        x = self.conv1(x) # [20, 64, 112, 112]
        x = self.bn1(x)
        x = self.relu(x)
        x, indices = self.maxpool(x) # [20, 64, 56, 56]

        x_l1 = self.layer1(x) # [20, 64, 56, 56]
        x_l2 = self.layer2(x_l1) # [20, 128, 28, 28]
        x_l3 = self.layer3(x_l2) # [20, 256, 14, 14]
        x_l4 = self.layer4(x_l3) # [20, 512, 7, 7]

        x = self.avgpool(x_l4) # [20, 512, 1, 1]
        x = torch.flatten(x, 1) # [20, 512]
        x = self.fc(x) # [20, 2048]

        return x, indices, x_l1, x_l2, x_l3, x_l4
