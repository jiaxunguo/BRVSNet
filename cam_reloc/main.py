...
log_fd = os.path.join(args.save_dir, '{}/{}_{}/'.format(args.scene, args.loss, args.num_coeff))
if not os.path.exists(log_fd):
    os.makedirs(log_fd)
log_fn = os.path.join(log_fd, '{}_log_eval_stage{}.txt'.format(args.scene, args.stage))
fid = open(log_fn, 'w')
        
print('Train Evaluation')
fid.write('Train Evaluation')
results, X, Y, _ = model.eval(train_loader)


hist = eval.compute_pose_histogram(thresholds, results['rotations'], results['translations'], results['labels'])
print(hist)
fid.write(np.array2string(hist))

print('Test Evaluation')
fid.write('Test Evaluation')
results, X_, Y_, Y_net = model.eval(val_loader)
